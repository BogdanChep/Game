...
/**
 * Класс Game представляет собой реализацию графического интерфейса игры.
...
public class Game extends JFrame {
        ...
        JButton expandButton = createButton("Захватить территорию", e -> {
            if (canPerformAction()) {
                ...
                if (neededPeasants <= player.getPeasants() && player.getWater() >= neededPeasants && player.getRice() >= neededPeasants) {
                    player.expandTerritory(map, x, y);
    ...
    private void showResourceGraphs() {
        ...
        JDialog dialog = new JDialog(this, "Графики ресурсов", true);
    ...
    private JFreeChart createDataset() {
        ...
        int totalDays = Math.max(playerWaterHistory.size(), aiWaterHistory.size());

        for (int i = 0; i < totalDays; i++) {
            if (i < playerWaterHistory.size()) {
                dataset.addValue(playerWaterHistory.get(i), "Вода игрока", "День " + (i + 1));
                dataset.addValue(playerRiceHistory.get(i), "Рис игрока", "День " + (i + 1));
                dataset.addValue(playerPeasantsHistory.get(i), "Крестьяне игрока", "День " + (i + 1));
            }
            if (i < aiWaterHistory.size()) {
                dataset.addValue(aiWaterHistory.get(i), "Вода ИИ", "День " + (i + 1));
                dataset.addValue(aiRiceHistory.get(i), "Рис ИИ", "День " + (i + 1));
                dataset.addValue(aiPeasantsHistory.get(i), "Крестьяне ИИ", "День " + (i + 1));
            }
        }

        return ChartFactory.createLineChart(
                "Ресурсы по дням", // Заголовок
                "День",           // Ось X
                "Количество",     // Ось Y
                dataset,          // Данные
                ...
                true,             // Легенда
                true,             // Примечания
                false             // Панель для обработки событий
        );
    }
